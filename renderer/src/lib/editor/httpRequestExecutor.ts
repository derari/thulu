import type { HttpSection, ParsedHttpFile } from './httpParser';
...
function substituteVariablesRecursive(
    text: string,
...
function findSectionByLineNumber(sections: HttpSection[], lineNumber: number): HttpSection | null {
    return (
        sections.find((s) => lineNumber >= s.startLineNumber && lineNumber < s.endLineNumber) ||
        null
    );
}

function extractBodyFromSection(
    lines: string[],
    bodySection: { startLineNumber: number; endLineNumber: number } | undefined
): string | null {
    if (!bodySection) {
        return null;
    }
...
async function loadEnvironmentVariables(
    ...
    try {
        return await getEnvironmentVariablesMap(environmentName, fileDirectory, collectionPath);
    } catch (error) {
