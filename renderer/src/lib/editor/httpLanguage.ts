	...
	for (const section of parsedFile.sections) {
		if (section.postScripts) {
			for (const postScript of section.postScripts) {
				if (lineNumber >= postScript.startLineNumber && lineNumber < postScript.endLineNumber) {
					return true;
				}
			}
		}
		...
		language: StreamLanguage.define<HttpState>({
			...
			token(stream, state) {
				...
				if (stream.sol()) {
					...
					if (state.mode === 'request' && (state.state === 'preamble' || state.state === 'section-title') && stream.match(/^@/)) {
						return 'strong';
					}
